...
$type   = get_field('csa_hero_type', 'option') ?: 'video';
$webm   = get_field('csa_hero_video_webm', 'option');
$mp4    = get_field('csa_hero_video_mp4', 'option');
$poster = get_field('csa_hero_video_poster', 'option');
$image  = get_field('csa_hero_image', 'option');

$poster_url = is_array($poster) ? ($poster['url'] ?? '') : '';
$cursor_enabled = (bool) get_field('csa_hero_cursor_enabled', 'option');
$cursor_img = get_field('csa_hero_cursor_image', 'option');
...
<section class="relative w-full min-h-screen overflow-hidden"<?php echo $cursor_attrs; ?>>
  <?php if ($type === 'video' && ((is_array($webm) && !empty($webm['url'])) || (is_array($mp4) && !empty($mp4['url'])))): ?>
    <video
